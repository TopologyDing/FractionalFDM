function[]=fractional_quarter_circular_inclusion_matrix_division(filename_1,filename_2,filename_3)
    dbstop if error
    ...
    for i=2:1:high_x
        for j=high_y:-1:2
            ...
            elseif(in_fic_for_out(i,j)==3)
                ...
                in_fic_for_out_ux(index,3)=in_fic_for_out_ux(index,1)+in_fic_for_out_ux(index,2)-d_d(i,j,1);
                in_fic_for_out_uy(index,3)=in_fic_for_out_uy(index,1)+in_fic_for_out_uy(index,2)-d_d(i,j,2);
                in_index_x(index)=i;
                in_index_y(index)=j;
                index=index+1;
            ...
            elseif(out_fic_for_in(i,j)==4)
                ...
                out_fic_for_in_ux(index_1,3)=out_fic_for_in_ux(index_1,1)+out_fic_for_in_ux(index_1,2)-d_d(i,j,1);
                out_fic_for_in_uy(index_1,3)=out_fic_for_in_uy(index_1,1)+out_fic_for_in_uy(index_1,2)-d_d(i,j,2);
                out_index_x(index_1)=i;
                out_index_y(index_1)=j;
                index_1=index_1+1;
            end
	...
	for i=1:1:length(in_fic_for_out_ux)
		...
		if(out_cross_boun_mark(index_x,index_y,1)==3)
            in_fic_for_out_ux(i,1)=d_d(index_x-1,index_y,1)+d_d(index_x,index_y-1,1)-d_d(index_x,index_y,1);
            in_fic_for_out_uy(i,1)=d_d(index_x-1,index_y,2)+d_d(index_x,index_y-1,2)-d_d(index_x,index_y,2);
		end
    ...
    for i=1:1:length(out_fic_for_in_ux)
        ...
        if(in_cross_boun_mark(index_x,index_y,1)==2)
            out_fic_for_in_ux(i,1)=d_d(index_x+1,index_y,1)+d_d(index_x,index_y+1,1)-d_d(index_x,index_y,1);
            out_fic_for_in_uy(i,1)=d_d(index_x+1,index_y,2)+d_d(index_x,index_y+1,2)-d_d(index_x,index_y,2);
        end
    ...
    coeff_matrix=zeros(2*max_x*max_y);
	for i=1:1:max_x
		for j=1:1:max_y
			for k=1:1:2
                ...
                for p=1:1:number_of_variable
                    ...
                    sub_num1=str2double(string_1);
                    sub_num2=str2double(string_2);
                    if(str_variable(2)=='x')% 1 stands for Ux; 2 stands for Uy;
                        coeff=coeffs(equilibrium(i,j,k),Ux(sub_num1,sub_num2));
                        if(size(coeff,2)==1)
                            coeff_matrix(((j-1)*max_x+i-1)*2+k,((sub_num2-1)*max_x+sub_num1-1)*2+1)=coeff(1);
                        else
                            coeff_matrix(((j-1)*max_x+i-1)*2+k,((sub_num2-1)*max_x+sub_num1-1)*2+1)=coeff(2);
                        end
                    else
                        coeff=coeffs(equilibrium(i,j,k),Uy(sub_num1,sub_num2));
                        if(size(coeff,2)==1)
                            coeff_matrix(((j-1)*max_x+i-1)*2+2,((sub_num2-1)*max_x+sub_num1-1)*2+2)=coeff(1);
                        else
                            coeff_matrix(((j-1)*max_x+i-1)*2+2,((sub_num2-1)*max_x+sub_num1-1)*2+2)=coeff(2);
                        end
                    end
                end
			end
	...
	coefficient_b=zeros(2*max_x*max_y,1);
    coefficient_b(2*(1:max_y)-1)=2000;
    
    displacement=coeff_matrix\coefficient_b;
    displacement_x=coeff_matrix(1:2:2*max_x*max_y);
    displacement_y=coeff_matrix(2:2:2*max_x*max_y);
    ...
    scatter3(x_posi_col,y_posi_col,stress_x_col,15,stress_x_col,'filled');
    ...
    save 'new-1-0-0.03.mat';
    xdisp=fopen(filename_1,'wt');
    ydisp=fopen(filename_2,'wt');
    disp=fopen(filename_3,'wt');
    for i=1:1:max_x
        for j=1:1:max_y
            fprintf(xdisp,'%d %d %.32f\n',[i j vpa(displacement(i,j,1))]);
            fprintf(ydisp,'%d %d %.32f\n',[i j vpa(displacement(i,j,2))]);
            fprintf(disp,'%d %d %.32f\n',[i j vpa(total_disp(i,j))]);
        end
    end
    fclose(xdisp);
    fclose(ydisp);
